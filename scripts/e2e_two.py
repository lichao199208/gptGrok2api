...
requests.post(BASE + "/api/register", headers=H, json={"total": 2, "mode": "total"}, timeout=30, verify=False)
...
for i in range(150):
    ...
    for log in cfg.get("logs") or []:
        ...
        if key in seen:
            continue
        seen.add(key)
        print(f"[{log.get('time')}] {log.get('level')} {text[:240]}")
    if stats.get("running") == 0 and stats.get("done") is not None and stats.get("done", 0) >= 2:
        break
...
print("success:", stats.get("success"), "fail:", stats.get("fail"), "done:", stats.get("done"), "success_rate:", stats.get("success_rate"))
