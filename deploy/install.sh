...
REPO_OWNER="${REPO_OWNER:-AuuCoder}"
REPO_NAME="${REPO_NAME:-gptGrok2api}"
BRANCH="${BRANCH:-main}"
...
usage() {
  ...
  curl -fsSL https://raw.githubusercontent.com/AuuCoder/gptGrok2api/main/deploy/install.sh | sudo bash
EOF
...
EOF
  ...
  --git-repo-url your-repository-url
  --git-token ghp_xxx
  --git-branch main
...
default_image() {
  ...
  image_owner="$(printf '%s' "${REPO_OWNER}" | tr '[:upper:]' '[:lower:]')"
  image_name="$(printf '%s' "${REPO_NAME}" | tr '[:upper:]' '[:lower:]')"

  if [[ "${BRANCH}" =~ ^v?[0-9] ]]; then
    printf 'ghcr.io/%s/%s:%s' "${image_owner}" "${image_name}" "${BRANCH}"
    return
  fi

  printf 'ghcr.io/%s/%s:latest' "${image_owner}" "${image_name}"
}
